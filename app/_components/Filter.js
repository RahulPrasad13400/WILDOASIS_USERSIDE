...
export default function Filter() {
  ...
  return (
    <div className="border border-primary-800 flex">
      ...
      <button onClick={()=>handleFilter('small')} className="px-5 py-2 hover:bg-primary-700">1&mdash;3 guests</button>
      <button onClick={()=>handleFilter('medium')} className="px-5 py-2 hover:bg-primary-700">4&mdash;7 guests</button>
      <button onClick={()=>handleFilter('large')} className="px-5 py-2 hover:bg-primary-700">8&mdash;12 guests</button> */}
...
function Button({filter, handleFilter, activeFilter, children}){
    return <button onClick={()=>handleFilter(filter)} className={`px-5 py-2 hover:bg-primary-700 ${filter === activeFilter ? 'bg-primary-700 text-primary-50' : ""}`}>{children}</button>
}
